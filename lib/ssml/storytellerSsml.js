...
function convertStoryToSsml(storyText) {
  ...
  for (const block of blocks) {
    ...
    if (hashMatch || isChapter) {
      ...
      let endText = escapeXml(endMatch[1]);
      if (!/[.!]$/.test(endText)) endText += '.';
      endText = endText.charAt(0).toUpperCase() + endText.slice(1);
      rendered.push({
        kind: 'end',
        ssml: `<break time="2.0s"/><prosody rate="x-slow" pitch="-3st" volume="-2dB">${endText}</prosody>`,
      });
    } else {
...
function convertStoryToPlainText(storyText) {
  ...
  for (const block of blocks) {
    ...
    if (hashMatch || isChapter) {
      ...
      let endText = endMatch[1];
      if (!/[.!]$/.test(endText)) endText += '.';
      endText = endText.charAt(0).toUpperCase() + endText.slice(1);
      rendered.push('..........     ' + endText.replace(/\s+/g, '.....   '));
    } else {
