...
test('"The End" gets maximum gravity, and does not double up the paragraph pause', () => {
  const ssml = convertStoryToSsml('Body text.\n\nThe End');
  assert.equal(
    ssml,
    '<speak>Body text.<break time="2.0s"/>' +
      '<prosody rate="x-slow" pitch="-3st" volume="-2dB">The End.</prosody></speak>'
  );
});
...
test('full story: title, paragraphs, dashes, aside, emphasis, mixed dialogue, and The End', () => {
  ...
  const expected =
    ...
    '<break time="2.0s"/><prosody rate="x-slow" pitch="-3st" volume="-2dB">The End.</prosody>' +
    '</speak>';
